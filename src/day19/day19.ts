export const step1 = (input: string) => {
   ...
   return blueprints.reduce((acc, blueprint, index) => {
      let states = [
         { robots: { ore: 1, clay: 0, obsidian: 0, geode: 0 }, resources: { ore: 0, clay: 0, obsidian: 0, geode: 0 } },
      ];
      ...
      for (let minute = 0; minute < 24; minute++) {
         ...
         let fitnesses = newStates.map((state) => {
            return (
               (state.resources.geode + (24 - minute) * state.robots.geode) * 10000000 +
...
export const step2 = (input: string) => {
   ...
   return blueprints.reduce((acc, blueprint, index) => {
      ...
      for (let minute = 0; minute < 32; minute++) {
         ...
         let fitnesses = newStates.map((state) => {
            return (
               (state.resources.geode + (32 - minute) * state.robots.geode) * 10000000 +
               state.robots.obsidian * 10000 +
               state.robots.clay * 100 +
               state.robots.ore
            );
